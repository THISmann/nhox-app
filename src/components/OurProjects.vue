<template>
  <div class="grid grid-cols-1 md:grid-cols-3 bg-black gap-4 p-4">
    <div v-for="(image, index) in articles" :key="index" class="relative border rounded-lg mb-2 h-96 bg-cover bg-center"
      :style="{ backgroundImage: `url(${image.imgSrc})` }">

      <div class="absolute inset-0 bg-black bg-opacity-50 flex items-center justify-center">
        <div class="text-center">
          <h1 class="text-white text-xl font-semibold mb-4">{{ image.title }}</h1>
          <router-link :to="{ name: 'RealisationItem', params: { id: image._id } }" class="text-white underline">
            Visit Link
          </router-link>
        </div>
      </div>
